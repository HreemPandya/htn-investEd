...
interface VideoGenerationJob {
  id: string
  status: 'queued' | 'generating' | 'rendering' | 'uploading' | 'ready' | 'error'
  url?: string
  error?: string
  timings?: {
    t0_start: number
...
const VideoTestPage = ({ userData, onNext }: VideoTestPageProps) => {
  ...
  const [testPrompt, setTestPrompt] = useState("How to start investing as a student")
        ...
        try {
          ...
          if (statusData.status === "ready") {
            ...
            if (statusData.timings) {
              addLog(`⏱️ Total time: ${statusData.timings.total_ms}ms (${(statusData.timings.total_ms / 1000).toFixed(1)}s)`)
            }
      ...
      <div className="px-6 pb-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold mb-2" style={{ color: '#23231A' }}>
          ...
          {/* Test Controls */}
          <div className="bg-white rounded-2xl p-6 mb-6 shadow-sm border" style={{ borderColor: '#E5E7EB' }}>
            <h2 className="text-xl font-semibold mb-4" style={{ color: '#23231A' }}>Test Configuration</h2>
          ...
          {currentJob && (
            <div className="bg-white rounded-2xl p-6 mb-6 shadow-sm border" style={{ borderColor: '#E5E7EB' }}>
              <h2 className="text-xl font-semibold mb-4" style={{ color: '#23231A' }}>Current Job Status</h2>
              
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium" style={{ color: '#91918D' }}>Job ID:</span>
                  <span className="font-mono text-sm" style={{ color: '#23231A' }}>{currentJob.id}</span>
                </div>

                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium" style={{ color: '#91918D' }}>Status:</span>
                ...
                {currentJob.timings && (
                  <div className="space-y-2">
                    <h3 className="text-sm font-semibold" style={{ color: '#23231A' }}>Timing Breakdown:</h3>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      {currentJob.timings.llm_ms && (
                        <div className="flex justify-between">
                          <span style={{ color: '#91918D' }}>LLM Generation:</span>
                          <span style={{ color: '#23231A' }}>{currentJob.timings.llm_ms}ms</span>
                        </div>
                      )}
                      {currentJob.timings.render_ms && (
                        <div className="flex justify-between">
                          <span style={{ color: '#91918D' }}>Video Rendering:</span>
                          <span style={{ color: '#23231A' }}>{currentJob.timings.render_ms}ms</span>
                        </div>
                      )}
                      {currentJob.timings.upload_ms && (
                        <div className="flex justify-between">
                          <span style={{ color: '#91918D' }}>File Upload:</span>
                          <span style={{ color: '#23231A' }}>{currentJob.timings.upload_ms}ms</span>
                        </div>
                      )}
                      {currentJob.timings.total_ms && (
                        <div className="flex justify-between font-semibold border-t pt-2">
                          <span style={{ color: '#23231A' }}>Total Time:</span>
                          <span style={{ color: '#005DAA' }}>{currentJob.timings.total_ms}ms ({(currentJob.timings.total_ms / 1000).toFixed(1)}s)</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {currentJob.url && (
                  <div className="space-y-2">
                    <h3 className="text-sm font-semibold" style={{ color: '#23231A' }}>Generated Video:</h3>
                ...
                {currentJob.meta?.script && (
                  <div className="space-y-2">
                    <h3 className="text-sm font-semibold" style={{ color: '#23231A' }}>Generated Script:</h3>
                    <div 
          ...
          {logs.length > 0 && (
            <div className="bg-white rounded-2xl p-6 shadow-sm border" style={{ borderColor: '#E5E7EB' }}>
              <h2 className="text-xl font-semibold mb-4" style={{ color: '#23231A' }}>Generation Logs</h2>
              <div 
